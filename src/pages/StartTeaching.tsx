...
const StartTeaching = () => {
  ...
  const handleAvailabilityToggle = (value: string, type: 'days' | 'times') => {
    setFormData(prev => ({
      ...prev,
      availability: {
        ...prev.availability,
  ...
  const handleSessionTypeToggle = (sessionType: string) => {
    setFormData(prev => ({
      ...
      sessionTypes: prev.sessionTypes.includes(sessionType)
        ? prev.sessionTypes.filter(type => type !== sessionType)
  ...
  const handleSubmit = async () => {
    setLoading(true);
    try {
      // Process skills wanted into the expected format
      const skillsWanted = formData.whatYouWant.split(',').map(skill => ({
        name: skill.trim(),
        level: 'Beginner',
        category: 'Other',
        priority: 'Medium',
      ...
      // Process skills offered into the expected format
      const skillsOffered = formData.skillsOffered.map(skill => ({
        name: skill,
        level: formData.experience === 'expert' ? 'Expert' : 
               formData.experience === 'advanced' ? 'Advanced' : 
               formData.experience === 'intermediate' ? 'Intermediate' : 'Beginner',
        category: skillCategories.includes(skill) ? skill : 'Other',
        experience: formData.credentials || 'Self-taught',
        rating: 0,
        sessionCount: 0
      }));
      ...
      const updateData = {
        ...
        preferredSessionDuration: formData.preferredSessionDuration
      };
      ...
      if (success) {
        toast({
          ...
          description: "Your teaching profile has been set up successfully. You can now start accepting students!",
        });
                ...
                <div className="space-y-6">
                  ...
                  <div>
                    ...
                    <div className="flex gap-2 mt-2">
                      <Input 
                        id="custom-skill" 
                        placeholder="e.g., Machine Learning, Pottery, etc."
                        value={customSkill}
              ...
              {currentStep === 2 && (
                <div className="space-y-6">
                  ...
                  <div>
                    ...
                    <Input 
                      placeholder="e.g., Computer Science Degree, AWS Certified, 5 years at Google"
              ...
              {currentStep === 3 && (
                <div className="space-y-6">
                          ...
                          {["Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Sunday"].map((day) => (
                            <div key={day} className="flex items-center space-x-2">
                              <Checkbox 
                                ...
                                onCheckedChange={() => handleAvailabilityToggle(day, 'days')}
                              />
                          ...
                          {["Morning", "Afternoon", "Evening", "Night"].map((time) => (
                            <div key={time} className="flex items-center space-x-2">
                              <Checkbox 
                                ...
                                onCheckedChange={() => handleAvailabilityToggle(time, 'times')}
                              />
                  ...
                  <div>
                    ...
                    <Select value={formData.availability.timezone} onValueChange={(value) => setFormData(prev => ({ ...prev, availability: { ...prev.availability, timezone: value } }))}>
                      <SelectTrigger>
                ...
                <div className="space-y-6">
                  <div>
                    ...
                    <div className="space-y-3">
                      {[
                        { id: "Video Call", icon: Video, title: "Video Calls", desc: "1-on-1 or group video sessions" },
                        { id: "Text Chat", icon: MessageCircle, title: "Text Chat", desc: "Answer questions via messaging" },
                        { id: "Workshop", icon: Users, title: "Workshops", desc: "Teach multiple students at once" },
                        { id: "Code Review", icon: CheckCircle, title: "Code/Work Review", desc: "Review and provide feedback" }
                      ].map((type) => (
                  ...
                  <div>
                    ...
                    <Select value={formData.preferredSessionDuration.toString()} onValueChange={(value) => setFormData(prev => ({ ...prev, preferredSessionDuration: parseInt(value) }))}>
                      <SelectTrigger>
                  ...
                  <div>
                    ...
                    <Textarea 
                      ...
                      onChange={(e) => setFormData(prev => ({ ...prev, whatYouWant: e.target.value }))}
                    />
                  </div>
                </div>
              )}
